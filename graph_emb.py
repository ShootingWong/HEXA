...
class Embed(nn.Module):
    ...
    def forward(self, node_ids, session_qids, session_dids, session_len, qcnt=None):
        ...
        session_intent[session_len == 1] = query_emb[session_len == 1]
...
class Aggregtor_HGT(nn.Module):
    def __init__(self, features, config, nodetype_dict, sample_prefix,
        ...
        self.neighbor_dict = pickle.load(open(config['neib_dict_addr'], 'rb')) 
        self.neighbor_freq_dict = pickle.load(open(config['neib_freq_dict_addr'], 'rb')) 
    ...
    def sample_neighs(self, nodes, freq, size):
        '''
        deal with the problem of sampling fathers
        '''
        ...
        if size == -1 or len(nodes) <= size:
            ...
            new_nodes = list(np.random.choice(nodes, size, p=freq, replace=False))
            return new_nodes
    ...
    def session_graph(self, query_emb, session_qids, session_dids, session_len):
        ...
        session_mask_d = torch.where(mask < (session_len-1).unsqueeze(-1), 1, 0)  # batch, session_len
        ...
        session_len_ext = (session_len-1).unsqueeze(-1).unsqueeze(-1).repeat([1, max_sess_len, max_sess_len])
        pre_mask_d = pre_mask.scatter(2, session_len_ext, 0)
        pre_mask_d = pre_mask_d.scatter(1, session_len_ext, 0)

        pre_edge_mask = pre_mask.sum(2) == 0
        back_mask = torch.where(mask.unsqueeze(1).repeat([1, max_sess_len, 1]) > pre_index.unsqueeze(-1), 1, 0) \
                    * session_mask.unsqueeze(1) * session_mask.unsqueeze(-1)  # batch, session_len, session_len
        back_mask_d = back_mask.scatter(2, session_len_ext, 0)
        back_mask_d = back_mask_d.scatter(1, session_len_ext, 0)
        ...
        click_mask = click_mask.masked_fill((1-session_mask_d).bool().unsqueeze(-1), 0)
        ...
        all_embs[indices] = self.features(input[indices], 1)  # We only use the encoder1 to produce the search intent, whose feature is bert

        querys_embs = all_embs[:batch * max_sess_len]
        docs_embs = all_embs[batch * max_sess_len:]#.view(batch, max_sess_len, -1)  # batch * session_len, hidden_size

        output_embed_d_1, _ = self.session_graph_message(session_dids, docs_embs, all_embs, pre_mask_d, back_mask_d, click_mask, blank, batch, max_sess_len, type=1)
        output_embed_q_1, _ = self.session_graph_message(session_qids, querys_embs, all_embs, pre_mask, back_mask, click_mask, blank, batch, max_sess_len, type=0)
        output_embed_q_1 = self.drop(output_embed_q_1)
        output_embed_d_1 = self.drop(output_embed_d_1)
        
        all_embs_new = torch.cat([output_embed_q_1.view(batch * max_sess_len, -1), output_embed_d_1.view(batch * max_sess_len, -1)], dim=0)
        output_embed_d, repeat = self.session_graph_message(session_dids, output_embed_d_1.view(batch * max_sess_len, -1), all_embs_new, pre_mask_d, back_mask_d, click_mask, blank, batch, max_sess_len, type=1)
    ...
    def forward(self, nodes, step, loc=None):
        ...
        for i in range(self.edge_num):
            samp_neighs = [
                self.sample_neighs(list(self.neighbor_dict[i][int(node)]), self.neighbor_freq_dict[i][int(node)],
                                self.sample_size) + [int(node)] \
                    if int(node) in self.neighbor_dict[i] else [int(node)] for node in nodes]
            samp_neighs_lists.append(samp_neighs)
        '''
    ...
    def message_process(self, nodes, samp_neighs_lists, flat_neigh, node2ind, embed_matrix, step):
        ...
        if step != 1: output_embed = self.drop(output_embed)
